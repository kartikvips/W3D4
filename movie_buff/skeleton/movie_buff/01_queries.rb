...
def harrison_ford
  ...
  Movie.joins(:actors).select(:id,:title).where(actors: {name:'Harrison Ford'}).where('castings.ord != 1')
end
...
def biggest_cast
  ...
  Movie.joins(:actors).select(:id, :title).group(:id).order('COUNT(actor_id) DESC').limit(3)
end
...
def directed_by_one_of(them)
  ...
  Movie.joins(:director).select('movies.id, movies.title').where('name IN (?)', them)
